...
const handlers = {
  "TNE_FT_VAGDATA": (feature, res, mode) => {
    ...
    const styles = [new Style({
      ...
      stroke: getStroke(`var(--map-road-${vagtyp})`, width, vagtyp === 'ovrigvag' ? [10, 12] : undefined)
    })];
    ...
    if (res < 9) {
      styles.push(new Style({
        ...
        text: new Text({
          ...
          fill: getFill("var(--map-text-fill)"), stroke: getStroke("var(--map-text-stroke)", 4)
        })
      }));
    }
  ...
  "traktor": (feature, res, mode) => mode == 0 ? new Style({ stroke: getStroke("var(--map-road-traktor)", 3, [10, 10], "square") }) : null,
  ...
  "vaglinje": (feature, res, mode) => {
    ...
    return new Style({
      ...
      stroke: getStroke("var(--map-road-belagd)", roadWidth[feature.get("objekttyp")] || 3),
      text: new Text({
  ...
  "hydrolinje": (feature) => new Style({
    stroke: getStroke("var(--map-water-line)", Number(feature.get("storleksklass")) || 3, undefined, "round")
  }),

  "skyddadnatur": (feature, r, mode) => mode == 0 ? new Style({
    zIndex: 5, stroke: getStroke(`var(--map-protected-${feature.get("objekttyp")})`, 4, dashPolygon.includes(feature.get("objekttyp")) ? [10, 10] : undefined)
  }) : null,

  "militart_omrade": (feature) => new Style({
    zIndex: 5, stroke: getStroke(`var(--map-military)`, 4, dashPolygon.includes(feature.get("objekttyp")) ? [10, 10] : undefined)
  }),

  "byggnad": () => new Style({ zIndex: 5, fill: getFill("#7d7d7d") }),
  "landningsbana": (feature) => new Style({ zIndex: 15, fill: getFill("var(--map-runway)") }),
  ...
  "textpunkt": (feature, res, mode) => {
    ...
    return new Style({
      ...
      text: new Text({
        ...
        fill: getFill(kategoriColor || "var(--map-text-fill)"), stroke: getStroke("var(--map-text-stroke)", (feature.get("textstorleksklass") * 0.3) + 3)
      })
    });
  },
